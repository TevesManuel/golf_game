...
static mut WINDOWS_FOCUSED: Lazy<HashMap<String, bool>> = Lazy::new(|| {
    HashMap::new()
});
...
impl Window
{
    ...
    fn has_focus(&self) -> bool
    {
        unsafe
        {
            for focused in WINDOWS_FOCUSED.iter_mut()
            {
                if *focused.1
                {
                    return true;
    ...
    pub fn frame(&mut self)
    {
        ...
        unsafe
        {
            WINDOWS_FOCUSED.insert(self.title.clone(), self.window.has_focus());
        }
